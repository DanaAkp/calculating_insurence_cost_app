...
async def create_a_new_tariff_plan(data: TariffPlanDataIN):
...
async def delete_tariff_plan_by_id(tariff_plan_id: str):
    result = await service.delete_tariff_plane(tariff_plan_id)
    return result


@tariff_plan_router.post('/upload')
async def upload_tariff_plans_json(data: JSONTariffPlansDataIN):
